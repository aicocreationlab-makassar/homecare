...
(async()=>{
 ...
 try{
  await go('admin/settings');await page.waitForURL('**/admin/login');
  ...
  await fill('password','sa3demo2026');await page.locator('button[type=submit]').click();await page.waitForURL('**/admin/dashboard');await page.locator('.kpi-grid').waitFor();assert.equal(await page.locator('.donut-inner strong').textContent(),'8');results.push('Login gate, invalid credentials, seed 8 fictitious records');
  await go('assessment');await next();assert(await page.locator('[aria-invalid=true]').count()>0);
  ...
  await page.locator('[name="consent.initialAssessment"]').check();await page.locator('[name="consent.privacy"]').check();await next();await page.waitForURL('**/assessment-success?id=*');await page.locator('#send-whatsapp').waitFor();
  ...
  await go('admin/patients');await fill('search','Pasien <Uji>');await page.waitForTimeout(240);assert.match(await page.locator('#result-count').textContent(),/1 dari 9/);await select('status','Selesai');assert.match(await page.locator('#result-count').textContent(),/0 dari 9/);await page.locator('button[type=reset]').click();await page.waitForTimeout(80);assert.match(await page.locator('#result-count').textContent(),/9 dari 9/);
  await go(`admin/patient-detail?id=${id}`);await select('status','Dihubungi');await page.locator('#status-form button').click();await page.waitForFunction(()=>document.querySelector('#header-status').textContent.includes('Dihubungi'));await fill('note','Catatan admin uji & aman');await page.locator('#note-form button').click();await page.waitForFunction(()=>document.querySelector('#timeline').textContent.includes('Catatan admin diperbarui'));await page.reload();await page.locator('#note-form').waitFor();assert.equal(await page.inputValue('[name=note]'),'Catatan admin uji & aman');assert.match(await page.locator('#header-status').textContent(),/Dihubungi/);results.push('Admin record visible, search/filter/reset, status + note + activity persistence');
  await go('admin/settings');await fill('whatsapp','081234567891');await page.locator('#settings-form button').click();await page.waitForTimeout(100);await go(`assessment-success?id=${id}`);assert(new URL(await page.locator('#send-whatsapp').getAttribute('href')).pathname.endsWith('6281234567891'));results.push('WhatsApp settings applied to public summary');
  const routes=['','tentang','layanan','kontak','blog','assessment',`assessment-success?id=${id}`,'admin/login','admin/dashboard','admin/patients',`admin/patient-detail?id=${id}`,'admin/settings'];
  ...
  await page.setViewportSize({width:390,height:844});await go('');await page.locator('.menu-toggle').click();assert.equal(await page.locator('.menu-toggle').getAttribute('aria-expanded'),'true');await page.keyboard.press('Escape');assert.equal(await page.locator('.menu-toggle').getAttribute('aria-expanded'),'false');await screenshot({path:'tests/home-mobile.png',fullPage:true});
  await go('admin/dashboard');await page.locator('#admin-menu').click();assert.equal(await page.locator('#admin-menu').getAttribute('aria-expanded'),'true');await page.keyboard.press('Escape');assert.equal(await page.locator('#admin-menu').getAttribute('aria-expanded'),'false');await screenshot({path:'tests/admin-mobile.png',fullPage:true});
  await go('assessment');await screenshot({path:'tests/assessment-mobile.png',fullPage:true});await page.setViewportSize({width:1440,height:1000});await go('');await screenshot({path:'tests/home-desktop.png',fullPage:true});await screenshot({path:'tests/home-hero.png'});await go('admin/dashboard');await screenshot({path:'tests/admin-desktop.png',fullPage:true});results.push('Mobile public navigation + admin drawer; screenshot artifacts');
  await go(`admin/patient-detail?id=${id}`);await page.locator('#delete-record').click();await page.locator('dialog button[value=cancel]').click();assert(await page.locator('#delete-record').count());await page.locator('#delete-record').click();await page.locator('dialog button[value=confirm]').click();await page.waitForURL('**/admin/patients');await go(`assessment-success?id=${id}`);assert.match(await page.locator('h1').textContent(),/tidak ditemukan/);
  await go('admin/settings');await page.locator('#clear-all').click();await fill('confirmation','HAPUS');await page.locator('dialog button[value=confirm]').click();await page.waitForFunction(()=>document.querySelector('#data-count').textContent.startsWith('0 assessment'));await go('admin/dashboard');assert.equal(await page.locator('.donut-inner strong').textContent(),'0');await page.reload();await page.locator('.kpi-grid').waitFor();assert.equal(await page.locator('.donut-inner strong').textContent(),'0');
  await go('admin/settings');await page.locator('#reset-demo').click();await page.locator('dialog button[value=confirm]').click();await page.waitForFunction(()=>document.querySelector('#data-count').textContent.startsWith('8 assessment'));results.push('Delete cancel/confirm, missing record handling, clear with typed confirmation, no automatic reseed, manual demo reset');
  await page.locator('#logout').click();await page.waitForURL('**/admin/login');await go('admin/patients');await page.waitForURL('**/admin/login');results.push('Logout and protected-route redirect');
