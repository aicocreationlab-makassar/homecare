...
(async()=>{
 ...
 try{
 await page.goto(base+'/assessment');await field('contact.name').fill('Pemesan Demo Mobile');await field('contact.whatsapp').fill('081234567890');await field('contact.relationship').selectOption('Anak');await noOverflow();await page.locator('#next').click();
 ...
 await field('consent.initialAssessment').check();await field('consent.privacy').check();await page.locator('#next').click();await page.waitForURL('**/assessment-success?id=*');await page.locator('#send-whatsapp').waitFor();await noOverflow();
 await page.route('**/assets/vendor/aos/aos.js',r=>r.abort());await page.goto(base+'/');await page.locator('.hero h1').waitFor();const hidden=await page.locator('[data-aos]').evaluateAll(els=>els.filter(e=>getComputedStyle(e).opacity==='0').length);assert.equal(hidden,0);await noOverflow();assert.deepEqual(errors,[]);
